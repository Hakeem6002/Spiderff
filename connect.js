...
if(usePairingCode && !Hanzx.authState.creds.registered) {
		const phoneNumber = await question('TSH_BUG_GOKU-V2 - Masukin Nomor Lu Tapi Pake Contohnya > 243:\n');
		const code = await Hanzx.requestPairingCode(phoneNumber.trim())
		console.log(`TSH_BUG_GOKU-V2 Ni Kodenya: ${code}`)
...
require("./TSH_BUG_GOKU-V2")(Hanzx, m, store)
} catch (err) {
console.log(err)
}
})
